...
function DraftInner({
  ...
  const grades = useMemo(() => computeGrades(picks, roster), [picks, roster]);

  async function onDraft(teamAb: string) {
    setSubmitting(true);
    setPickError(null);
    const result = await confirmPick(teamAb);
    setSubmitting(false);
    if (!result.ok) setPickError(result.error);
  }

  // Direct-URL edge case: someone visits /draft before the commissioner has started it. Gated on
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed", minWidth: 320 }}>
                  ...
                  <thead>
                    <tr style={{ background: "var(--color-neutral-900)" }}>
                      ...
                      {ODDS_SOURCES.map((s) => (
                        <th key={s} style={{ textAlign: "right", padding: "8px 4px", fontSize: 10, letterSpacing: ".04em", textTransform: "uppercase", fontFamily: "var(--font-heading)", color: "var(--color-neutral-500)", whiteSpace: "nowrap", borderBottom: "1px solid var(--color-divider)" }}>
                  ...
                  <tbody>
                    {grades.map((g, i) => {
                      const isMine = g.manager === currentUserId;
                      const isLeader = i === 0;
                      const isLast = i === grades.length - 1;
                      return (
                        <tr key={g.manager} style={{ background: isMine ? "var(--color-accent-900)" : "transparent", borderBottom: isLast ? "none" : "1px solid var(--color-divider)" }}>
                          <td style={{ padding: "9px 4px 9px 12px" }}>
                            <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
                              ...
                              <span style={{ fontSize: 12.5, color: isMine ? "var(--color-accent-200)" : "var(--color-text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                {displayName(g.manager)}
                              </span>
                            </div>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 2 }}>
                      ...
                      <div style={{ fontSize: 12, color: "var(--color-neutral-500)" }}>
                        {tm.div} · O/U <span style={{ fontFamily: "ui-monospace,monospace", color: tm.ou >= 10 ? "var(--color-accent-300)" : "var(--color-neutral-400)" }}>{tm.ou.toFixed(1)}</span>
                      </div>
                    </div>
                    {isTaken ? (
                      <div style={{ flex: "none", fontSize: 10.5, letterSpacing: ".04em", textTransform: "uppercase", color: "var(--color-neutral-300)", background: "var(--color-neutral-800)", borderRadius: 4, padding: "3px 8px" }}>
