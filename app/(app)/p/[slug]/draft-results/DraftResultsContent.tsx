...
type RoundPick = { pickNo: number; userId: string; teamAb: string | null };
...
export function DraftResultsContent({
  ...
  const draftOrder30 = useMemo(() => buildDraftOrder(roster), [roster]);
  ...
  const rounds = useMemo(() => {
    ...
    for (let r = 0; r < 3; r++) {
      ...
      for (let i = 0; i < perRound; i++) {
        ...
        roundPicks.push({ pickNo, userId: draftOrder30[pickNo - 1] ?? "", teamAb: pick?.teamAb ?? null });
      }
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, tableLayout: "fixed", minWidth: 320 }}>
                  ...
                  <thead>
                    <tr style={{ background: "var(--color-neutral-900)" }}>
                      ...
                      {ODDS_SOURCES.map((s) => (
                        <th key={s} style={{ textAlign: "right", padding: "8px 4px", fontSize: 10.5, letterSpacing: ".04em", textTransform: "uppercase", fontFamily: "var(--font-heading)", color: "var(--color-neutral-500)", whiteSpace: "nowrap", borderBottom: "1px solid var(--color-divider)" }}>
                        ...
                        <tr key={g.manager} style={{ background: isMine ? "var(--color-accent-900)" : "transparent", borderBottom: isLast ? "none" : "1px solid var(--color-divider)" }}>
                          <td style={{ padding: "9px 4px 9px 12px" }}>
                            <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
                              ...
                              <span style={{ fontSize: 13, color: isMine ? "var(--color-accent-200)" : "var(--color-text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                {nameByUserId[g.manager] ?? "Someone"}
                              </span>
                            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {rnd.picks.map((p) => {
                  ...
                  return (
                    <div
                      key={p.pickNo}
                      onClick={p.teamAb ? () => router.push(`/p/${slug}/team/${p.teamAb}`) : undefined}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: 12,
