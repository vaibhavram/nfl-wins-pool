...
import { pauseDraft } from "@/lib/season-server";

export async function POST(_req: Request, ctx: RouteContext<"/api/p/[slug]/draft/pause">) {
  ...
  if (ctxOrError.membership.role !== "commissioner") {
    return NextResponse.json({ ok: false, error: "Only the commissioner can pause the draft." }, { status: 403 });
  }
  ...
  const result = await pauseDraft(season.id);
