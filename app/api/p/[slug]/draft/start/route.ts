...
import { startDraft } from "@/lib/season-server";

export async function POST(_req: Request, ctx: RouteContext<"/api/p/[slug]/draft/start">) {
  ...
  const result = await startDraft(season.id);
