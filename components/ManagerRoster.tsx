 ...
 * the "My teams" tab (viewing yourself) and /manager/[name] (viewing anyone) so the two can
 * never drift out of sync with each other. */
export function ManagerRoster({ name, title }: { name: string; title: string }) {
  const router = useRouter();
  const { picks } = useDraft();
  const { standings, loading } = useStandings();
  const { results: simResults } = useSimulation();

  const teams = useMemo(() => rostersFromPicks(picks)[name] ?? [], [picks, name]);
  const wins = totalWins(teams, standings);
  const sim = simResults.find((r) => r.manager === name);
